...
import { TextInputField, TextareaField } from "@/components/shared/FormFields";
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            {isEditing ? "Update the author details below." : "Fill in the details to add a new author."}
          </DialogDescription>
        </DialogHeader>
        <form
          ...
          className="space-y-4"
        >
          <form.Field name="name" validators={{ onChange: authorFormSchema.shape.name }}>
            {(field) => <TextInputField field={field} label="Name" placeholder="Enter author name" required />}
          </form.Field>
          <form.Field name="bio" validators={{ onChange: authorFormSchema.shape.bio }}>
            {(field) => <TextareaField field={field} label="Biography" placeholder="Author biography..." rows={4} />}
          </form.Field>
          <DialogFooter>
